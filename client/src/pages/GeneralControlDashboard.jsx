        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map(item => {
              // Correção visual: Se tem responsável, considera Em Uso
              const displayStatus = item.currentHolder && item.status === 'disponivel' ? 'em_uso' : item.status;
              return (
              <div key={item.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${displayStatus === 'disponivel' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
